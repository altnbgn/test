...
def draw_result(image, boxes, text, timestamp):
    res_image = image
    for box in boxes:
        x = box[0]
        y = box[1]
        w = box[2] - box[0]
        h = box[3] - box[1]
        res_image = cv2.rectangle(res_image, (x, y), (x + w, y + h), (0, 0, 255), 3)
    cv2.putText(res_image, 'Oversized rocks: '+text, (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
    cv2.putText(res_image, timestamp, (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
